...
def iterate(args):
    folder, filename, home_path, config, threshold_lgn, ToRGC_image, ToRGC, coc, reduce_factor = args

    im = cv2.imread(os.path.join(folder, filename)) # Always loads images in BGR order
    im = im[:,:,::-1] # Reverse order to RGB
    if type(reduce_factor) is tuple:
    ...
    (ce, sc, beta, gamma) = lgn_statistics(im=im, coc=coc, ToRGC=ToRGC, config=config, file_name=None, force_recompute=True, cache=False, threshold_lgn=threshold_lgn, 
                                           compute_extra_statistics=False, verbose_filename=False, verbose=False, home_path=home_path, fov_imsize=imsize)
...
def run_LGNstatistics(rgc, coc, reduce_factor=1, config={'fov_gamma':5, 'viewing_dist':1}, sub=None):
    ...
    if rgc:
        ...
        # im = GCS(im)
        # out_size = out_size // 4
        # image_shape = tuple(np.array(im.shape)[:2] // 4) + (im.shape[2],)
        # shape = tuple(np.array(im.shape)[:2] // 4)
        ToRGC_image = ToRetinalGanglionCellSampling(out_size=out_size, image_shape=np.array(im).shape, series=1, n_processes=nproc)
        ToRGC = ToRetinalGanglionCellSampling(out_size=out_size, image_shape=np.array(im).shape[:2], series=1, n_processes=nproc)
    else:
        ToRGC_image = identity
        ToRGC = identity

    if type(reduce_factor) is not tuple:
        reduce_factor = 1/reduce_factor

    print('Start Computing LGN Statistics')
    # for filename in tqdm.tqdm(file_names, total=len(file_names)):
    #     filename, ce, sc, beta, gamma = iterate((folder, filename, home_path, config, threshold_lgn, ToRGC_image, ToRGC, coc, reduce_factor))
    ...
    with multiprocessing.Pool(nproc) as pool:
        results = list(tqdm.tqdm(pool.imap(iterate, [(folder, filename, home_path, config, threshold_lgn, ToRGC_image, ToRGC, coc, reduce_factor) for filename in file_names]), total=len(file_names)))
    ...
    print('Done Computing LGN Statistics')

    results = {'CE': ce_results, 'SC': sc_results, 'Beta': beta_results, 'Gamma': gamma_results, 'filenames': filenames, 'config': config}
    reduce_factor_string = ''
    if type(reduce_factor) is tuple:
        reduce_factor_string = f'_reduced-{reduce_factor[0]}x{reduce_factor[1]}'
    else:
        reduce_factor_string = "_reduced-" + str(reduce_factor) if reduce_factor > 1 else ""
    result_manager.save_result(result=results, filename=f'oads{"_gcs" if rgc else ""}{"_rgb" if not coc else ""}{reduce_factor_string}_lgn_statistics.pkl')
